...
(расшифрованы локально, faster-whisper). Фотографии — оттуда же:
интерьерные референсы, которые Мадина подобрала для сайта (латунные ручки,
арки, дерево, бархат) — они задают люксовый стиль оформления.
...
| Галерея «Детали» (подписи) | `site.details` |
| Цены | `site.pricing` |
...
```
src/
├─ content/site.ts     ← почти весь текст сайта (см. «Где править тексты»)
...
├─ assets/photos/      ← интерьерные фото (lux-*.jpg)
├─ assets/fonts/       ← Cormorant (самохостинг, кириллица + латиница)
